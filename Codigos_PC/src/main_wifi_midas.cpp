...
int main() {
    ...
    // Cargar clases y modelos
    const std::string classes_path = "C:/Users/andre/Documents/Trabajo_fin_master/Codigos_PC/models/classes.txt";
    const std::string yolo_path    = "C:/Users/andre/Documents/Trabajo_fin_master/Codigos_PC/models/yolov5n.torchscript";
    const std::string midas_path   = "C:/Users/andre/Documents/Trabajo_fin_master/Codigos_PC/models/traced_midas_model.pt";

  
    // Cargar clases y modelos
    std::vector<std::string> class_list = load_class_list(classes_path);
    torch::jit::script::Module yolo_model;
    torch::jit::script::Module midas_model;

    try {
        yolo_model = torch::jit::load(yolo_path);
        midas_model = torch::jit::load(midas_path);
    } catch (const c10::Error& e) {
        std::cerr << "Error al cargar los modelos TorchScript:\n" << e.what() << std::endl;
        return -1;
    }
    ...
    while (true) {
        ...
        detect_with_depth(img, yolo_model, midas_model, detections, class_list);

        for (const auto& d : detections) {
            cv::rectangle(img, d.box, cv::Scalar(0, 255, 0), 2);
            std::ostringstream label;
            label << class_list[d.class_id] << " " << std::fixed << std::setprecision(2)
                  << d.confidence << " D:" << d.depth;
            cv::putText(img, label.str(), cv::Point(d.box.x, d.box.y - 5),
...
//     cv::dnn::Net yolo_net = cv::dnn::readNet("C:/Users/andre/Documents/Trabajo_fin_master/Codigos_PC/models/yolov5n.onnx");
//     yolo_net.setPreferableBackend(cv::dnn::DNN_BACKEND_OPENCV);
//     yolo_net.setPreferableTarget(cv::dnn::DNN_TARGET_CPU);

//     torch::jit::script::Module midas_net = torch::jit::load("C:/Users/andre/Documents/Trabajo_fin_master/Codigos_PC/models/midas_v21_small_256_no_neg_fixed.pt");
