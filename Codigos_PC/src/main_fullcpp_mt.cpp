    ...
    while (true) {
        ...
        if (!current_depth.empty() && current_depth.cols > 0 && current_depth.rows > 0) {
            if (model == 0) {
                ...
                future = exponential_smoothing(current_depth, future, 0.5);
            }
        ...
        if(use_yolo  && !frame.empty() && !current_depth.empty())cv::imshow("YOLO + depth", frame);
        if(!depth_vis.empty() && !current_depth.empty())cv::imshow("Depth", depth_vis);
        cv::setMouseCallback("Depth", on_mouse_depth);
        depth_scaled_for_debug = current_depth.clone();
